...
const OrderList = () => {
  ...
  // 사용자 정보 가져오기
  const { user, isLoggedIn, refreshToken, isLoading: userContextLoading, logout } = useUser();
  
  // 디버깅용 로그
  console.log('OrderList - UserContext 상태:', {
    user: !!user,
    isLoggedIn,
  ...
  const handleModalClose = () => {
    ...
    if (modalState.modalType === 'alert' && 
        (modalState.alertMessage?.includes('세션이 만료되었습니다') || 
         ...
         modalState.alertMessage?.includes('로그인이 필요합니다'))) {
      if (logout) {
  ...
  const checkLoginStatus = () => {
    ...
    if (isLoggedIn) {
      ...
      if (!token || !refreshTokenValue) {
        console.log('UserContext isLoggedIn이 true이지만 토큰이 없음 - 로그아웃 상태로 처리');
        return false;
      }
    ...
    const hasRefreshToken = !!refreshTokenValue;
    ...
    // access_token과 refresh_token 모두 있어야 로그인 상태로 간주
  ...
  const debugTokenStatus = () => {
    ...
    console.log('🔍 OrderList - 토큰 상태 디버깅:', {
    ...
    if (accessToken) {
      try {
        ...
        console.log('🔍 토큰 디코딩 실패:', error.message);
      }
    }
  };
  ...
  const loadOrderData = useCallback(async () => {
    console.log('loadOrderData 함수 시작');
    ...
    console.log('loadOrderData에서 로그인 상태 확인:', loginStatus);
    ...
    try {
      ...
      try {
        ...
        if (error.response?.status === 401) {
          ...
          console.log('refreshToken 함수 확인:', { hasRefreshToken: !!refreshToken, refreshTokenType: typeof refreshToken });
          
          if (refreshToken) {
            try {
              console.log('토큰 갱신 시도 시작');
              const refreshSuccess = await refreshToken();
              console.log('토큰 갱신 결과:', refreshSuccess);
              
              if (refreshSuccess) {
                console.log('토큰 갱신 성공. API 재시도합니다.');
                // 토큰 갱신 성공 시 API 재시도
                try {
                  console.log('API 재시도 시작 - getUserOrders(10)');
                  const retryResponse = await orderApi.getUserOrders(10);
                  ordersData = retryResponse;
                  console.log('토큰 갱신 후 API 재시도 성공:', ordersData);
                } catch (retryError) {
                  console.error('토큰 갱신 후 API 재시도 실패:', retryError);
                  // 재시도 실패 시 401 에러를 다시 던져서 상위에서 처리
                  throw retryError;
                }
              } else {
                console.log('토큰 갱신 실패 - 401 에러를 상위로 전달');
                // 토큰 갱신 실패 시 401 에러를 다시 던져서 상위에서 처리
                throw error;
              }
            } catch (refreshError) {
              console.error('토큰 갱신 중 에러 발생:', refreshError);
              // 토큰 갱신 중 에러 발생 시 원래 401 에러를 다시 던져서 상위에서 처리
              throw error;
            }
          } else {
            console.log('토큰 갱신 함수를 사용할 수 없습니다 - 401 에러를 상위로 전달');
            // refreshToken 함수가 없는 경우 401 에러를 다시 던져서 상위에서 처리
            throw error;
          }
        } else {
          // 401 에러가 아닌 경우 에러를 다시 던져서 상위에서 처리
          throw error;
        }
      }
      
      // 새로운 API 응답 구조 확인 및 안전한 데이터 변환
      if (!ordersData || !ordersData.order_groups || !Array.isArray(ordersData.order_groups)) {
        console.log('API 응답에 order_groups가 없거나 배열이 아닙니다:', ordersData);
        // 주문이 없는 경우 빈 배열로 설정
        setOrderData({
          orders: [],
          total_count: 0,
      ...
      if (ordersData.order_groups.length === 0) {
        console.log('주문 그룹이 비어있습니다.');
        // 주문이 없는 경우 빈 배열로 설정
        setOrderData({
          orders: [],
          total_count: 0,
      ...
      setOrderData({
        ...
        total_count: ordersData.order_count || transformedOrders.length,
        page: 1,
        size: 20
      });
      ...
      if (error.response?.status === 401) {
        ...
        console.log('외부 에러 핸들러에서 refreshToken 확인:', { hasRefreshToken: !!refreshToken, refreshTokenType: typeof refreshToken });
        
        // 토큰 갱신이 실패했거나 불가능한 경우 로그인 모달 표시
        if (refreshToken) {
          console.log('토큰 갱신을 다시 시도합니다.');
          try {
            const refreshSuccess = await refreshToken();
            console.log('외부 에러 핸들러에서 토큰 갱신 결과:', refreshSuccess);
            
            if (!refreshSuccess) {
              console.log('토큰 갱신 최종 실패 - 로그인 모달 표시');
              setModalState(showLoginRequiredNotification());
              setLoading(false);
              return;
            }
          } catch (refreshError) {
            console.log('토큰 갱신 중 에러 발생 - 로그인 모달 표시');
            setModalState(showLoginRequiredNotification());
            setLoading(false);
            return;
          }
        } else {
          console.log('토큰 갱신 함수 없음 - 로그인 모달 표시');
          setModalState(showLoginRequiredNotification());
          setLoading(false);
          return;
        }
  ...
  }, [userContextLoading, refreshToken, navigate, setModalState, setLoading, setError, setOrderData]);
  ...
  }, [userContextLoading, isLoggedIn, loadOrderData]); // UserContext 상태 변화 감지
